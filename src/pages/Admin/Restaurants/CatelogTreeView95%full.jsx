...
const findProductById = (id) => {
  return products.find(p => p._id === id) || mainProducts.find(p => p._id === id);
};
...
// Set this new category as selected IMMEDIATELY
        setNavigationPath([newCategory]);
        setSelectedCategory(newCategory);
        setSelectedProduct(null);
        setCategoryDropdown(null); // Close any open dropdown
...
const handleSubcategorySubmit = async (formData) => {
  try {
    ...
    // === NEW: Automatically select and navigate to the newly created subcategory ===
    // Find the full path to the new subcategory
    const fullPath = findCategoryPath(updatedCategories, newCategory._id);
    if (fullPath) {
      setNavigationPath(fullPath);
      setSelectedCategory(newCategory);
      // Optional: also fetch products if it has any (though new one won't)
      // fetchProductsForCategory(newCategory._id);
    }
  ...
  try {
    ...
    // === IMPROVED: Smart navigation handling after deletion ===
    // We don't want to blindly reset everything — only adjust what's necessary

    let newNavigationPath = [...navigationPath];
    let newSelectedCategory = selectedCategory;

    // Case 1: The deleted category was currently selected
    if (selectedCategory?._id === categoryId) {
      // Try to fall back to the parent category
      const parentIndex = newNavigationPath.findIndex((cat, idx) =>
        idx < newNavigationPath.length - 1 &&
        newNavigationPath[idx + 1]?._id === categoryId
      );

      if (parentIndex >= 0) {
        // Keep the path up to and including the parent
        newNavigationPath = newNavigationPath.slice(0, parentIndex + 1);
        newSelectedCategory = newNavigationPath[parentIndex];
      } else {
        // No parent found → reset to root (main categories view)
        newNavigationPath = [];
        newSelectedCategory = null;
      }

      setSelectedProduct(null); // Always clear product when category is gone
    } 
    // Case 2: The deleted category was deeper in the open path (not selected, but open)
    else {
      const deletedIndex = newNavigationPath.findIndex(cat => cat._id === categoryId);
      if (deletedIndex >= 0) {
        // Truncate the path up to (but not including) the deleted category
        newNavigationPath = newNavigationPath.slice(0, deletedIndex);
        newSelectedCategory = newNavigationPath[newNavigationPath.length - 1] || null;
      }
    }

    // Apply the updated navigation state
  ...
  try {
    ...
    if (productModalData.mode === "create") {
      // ✅ FIX: For creation, pass ALL images (including File objects)
      // Don't filter them out - let createProduct handle the File vs URL distinction
      const preparedProductData = {
        ...productData,
        // Keep ALL images as they are (mix of Files and URLs)
        restaurantId: id,
        categoryId: productModalData.data?.categoryId, // null for main products
        ...(productData.availability === "time-based" && {
          availableAfterTime: productData.availableAfterTime,
          availableFromTime: null,
          availableToTime: null
        ...
        minimumOrderQuantity: productData.minQty || "",
        maximumOrderQuantity: productData.maxQty || "",
        active: productData.active !== undefined ? productData.active : true
      };
      
      // Debug: Check what images we're sending
      console.log('CREATE - Images to send:', preparedProductData.images);
      console.log('CREATE - File count:', preparedProductData.images?.filter(img => img instanceof File).length);
      console.log('CREATE - URL count:', preparedProductData.images?.filter(img => typeof img === 'string').length);
      
      // ✅ Call createProduct with ALL images
      const response = await createProduct(
        preparedProductData, 
        id, 
        preparedProductData.categoryId
      );
      
      const newProduct = formatProduct(response.data);
      
      // Add to appropriate state based on product type
      if (preparedProductData.categoryId) {
        // Regular category product
        setProducts(prev => [...prev, newProduct]);
        ...
        // Main product (store-level, no category)
        setMainProducts(prev => [...prev, newProduct]);
      }
      ...
      setTimeout(() => {
        if (preparedProductData.categoryId) {
          ...
          handleMainProductSelect(newProduct);
        }
      }, 100);
      
    } else {
      // EDIT mode logic - keep existing logic but ensure it works correctly
      const newImages = productData.images.filter((img) => img instanceof File);
      const existingImages = productData.images.filter((img) => typeof img === "string");
      
      const productDataForApi = {
        ...productData,
        images: existingImages, // Only URLs for API
        categoryId: productData.categoryId,
        ...(productData.availability === "time-based" && {
          availableAfterTime: productData.availableAfterTime,
          availableFromTime: null,
      ...
      // Debug
      console.log('UPDATE - New files:', newImages.length);
      console.log('UPDATE - Existing URLs:', existingImages.length);
      console.log('UPDATE - Images to remove:', productData.imagesToRemove?.length || 0);
      ...
      if (productData.categoryId) {
        ...
        // Main product (store-level)
        const updatedMainProducts = mainProducts.map(product =>
          product._id === productData._id
            ? updatedProduct
            : product
        );
    ...
    console.error("Error:", error);
    toast.error(`Failed to ${productModalData.mode} product: ${error.response?.data?.message || error.message}`);
  } finally {
    setLoading(prev => ({ ...prev, actions: false }));
  }
};
  ...
  try {
    ...
    // Remove from regular products
    setProducts(prev => prev.filter(p => p._id !== productId));

    // ← ADD THIS: Remove from main products too
    setMainProducts(prev => prev.filter(p => p._id !== productId));
...
{navigationPath.map((category, index) => (
  // For each category in the path, show its contents (subcategories + products)
          ...
          <div className="space-y-2">
            {category.children.map((subcategory) => (
              <div
                ...
                onClick={() => handleCategorySelect(subcategory)}
                className={`p-3 rounded-lg cursor-pointer transition-all ${
                  navigationPath[index + 1]?._id === subcategory._id
                    ? "bg-pink-50 border border-pink-300"
                    : "hover:bg-gray-50 border border-transparent hover:border-gray-200"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative group">
                      {/* Folder icon - also use the same condition for pink background */}
                      <div className={`p-1.5 rounded ${navigationPath[index + 1]?._id === subcategory._id ? 'bg-pink-200' : 'bg-gray-100'}`}>
                        <Folder className="h-4 w-4 text-pink-500" />
                      </div>
                      {/* Keep all your image badges exactly as they are */}
                      {subcategory.images && subcategory.images.length > 0 && (
                        <>
                          <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full overflow-hidden border-2 border-white bg-white shadow-sm">
                            <img src={subcategory.images[0]} alt="" className="w-full h-full object-cover" />
                          </div>
                          {subcategory.images.length > 1 && (
                            <div className="absolute -bottom-1 -left-1 w-4 h-4 rounded-full overflow-hidden border-2 border-white bg-white shadow-sm">
                              <img src={subcategory.images[1]} alt="" className="w-full h-full object-cover" />
                            </div>
                          )}
                      ...
                      {subcategory.images && subcategory.images.length > 0 && (
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-10">
                          <div className="bg-white rounded-lg shadow-xl border border-gray-200 p-3 w-64">
                            <div className="grid grid-cols-3 gap-2 mb-2">
                              {subcategory.images.slice(0, 6).map((img, idx) => (
                                <div key={idx} className="aspect-square overflow-hidden rounded-md">
                                  <img src={img} alt={`${subcategory.name} ${idx + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-200" />
                                </div>
                              ))}
                            </div>
                            {subcategory.images.length > 6 && (
                              <p className="text-xs text-gray-500 text-center">+{subcategory.images.length - 6} more images</p>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-gray-800">{subcategory.name}</div>
                      {/* Show counts only when selected - use same condition */}
                      {navigationPath[index + 1]?._id === subcategory._id && (
                        <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                          <span className={`px-1.5 py-0.5 rounded ${subcategory.active ? 'bg-green-100 text-green-800' : 'bg-gray-100'}`}>
                            {subcategory.active ? 'Active' : 'Inactive'}
                  ...
                  {/* ADD THIS SECTION - Right side with chevron and three-dots button */}
                  <div className="flex items-center gap-1">
                    {subcategory.children && subcategory.children.length > 0 && (
                      <ChevronRight className="h-4 w-4 text-gray-400" />
                    )}
                    {/* THREE-DOTS BUTTON FOR EDIT/DELETE ACTIONS */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const rect = e.currentTarget.getBoundingClientRect();
        ...
        >
          <div className="py-1">
            <button
              onClick={(e) => {
                e.stopPropagation();
                const product = findProductById(productDropdown);
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                const product = findProductById(productDropdown);
                if (product) handleToggleProductStatus(product._id, product.active);
                setProductDropdown(null);
              }}
              className="flex items-center w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100"
            >
              {findProductById(productDropdown)?.active ? (
                <>
                  <EyeOff className="h-4 w-4 mr-2" />
                  Disable Product
                </>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                const product = findProductById(productDropdown);
                if (product) handleArchiveProduct(product._id, !product.archived);
                setProductDropdown(null);
              }}
              className="flex items-center w-full px-4 py-2.5 text-sm text-orange-600 hover:bg-orange-50"
            >
              <Archive className="h-4 w-4 mr-2" />
              {findProductById(productDropdown)?.archived ? "Unarchive" : "Archive"}
            </button>
            <div className="border-t border-gray-200 my-1"></div>
            <button
              onClick={(e) => {
