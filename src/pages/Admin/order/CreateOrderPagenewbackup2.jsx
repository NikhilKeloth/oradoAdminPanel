...
export default function CreateOrderPage() {
  ...
  // Fetch restaurant menu by restaurantId
  ...
  const handleMerchantSelect = (merchant) => {
    ...
    setForm(prev => ({
      ...prev,
      restaurantId: merchant.id // Using 'id' from your API response
    }));
    setMerchantSearchTerm(merchant.name); // Using 'name' from your API response
    setShowMerchantDropdown(false);
    
    // Fetch restaurant menu for selected merchant
  ...
  const updateItemQuantity = (product, newQty) => {
    if (newQty < 0) return;
      ...
      if (existsIdx >= 0) {
        if (newQty === 0) {
          ...
          const items = prev.items.map((item, i) =>
            i === existsIdx ? { ...item, qty: newQty } : item
          );
          return { ...prev, items };
        }
      } else if (newQty > 0) {
  ...
  const addProductFromMerchant = (product) => {
    updateItemQuantity(product, getItemQuantity(product._id) + 1);
  };
  ...
  const decreaseProductQuantity = (product) => {
    ...
    updateItemQuantity(product, currentQty - 1);
  };
  ...
  async function handleSubmit(e) {
    ...
    if ((form.deliveryMode === "home_delivery" || form.deliveryMode === "pick_drop") && !form.address.trim()) {
      alert("Please provide a delivery address");
      return;
    }

    if (form.deliveryMode === "take_away" && (!form.pickupName.trim() || !form.pickupPhone.trim())) {
      alert("Please provide pickup name and phone");
      return;
    }
    ...
    try {
      ...
      // Here you would typically make the API call to create the order
      // await axios.post(`${API_BASE_URL}/admin/orders`, payload, {
      //   headers: {
            ...
            <div className="xl:col-span-3 space-y-4">
              ...
              <div className="bg-white rounded-xl shadow p-4 border border-orange-100">
                ...
                <div className="space-y-3">
                  <div className="space-y-1 relative" ref={merchantRef}>
              ...
              {/* Restaurant Menu Section - Only show when merchant is selected */}
              {selectedMerchant && (
                <div className="bg-white rounded-xl shadow p-4 border border-orange-100">
              ...
              {/* Order Items Card - For custom items */}
              <div className="bg-white rounded-xl shadow p-4 border border-orange-100">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    <div className="p-1.5 bg-orange-100 rounded-lg">
                    ...
                    <h2 className="font-semibold text-gray-800">Custom Order Items</h2>
                  </div>
