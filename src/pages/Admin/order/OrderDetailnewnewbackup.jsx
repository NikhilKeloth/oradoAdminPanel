...
import { FiEdit2, FiSave, FiX, FiMapPin, FiPlus, FiMinus, FiTrash2, FiSearch, FiNavigation, FiMap } from 'react-icons/fi';
...
const OrderDetails = () => {
  ...
  const fetchOrderDetails = useCallback(async () => {
    try {
      ...
      if (response) {
        ...
        setEditedAddress(response.restaurant?.address || {});
      }
    } catch (error) {
  ...
  const cancelEditing = () => {
    ...
    setEditedAddress(order.restaurant?.address || {});
  };
  ...
  const saveEditing = async () => {
    try {
      ...
      if (editingSection === 'additionalInfo') {
        ...
        updatedData.restaurantAddress = editedAddress;
      }
  ...
  // Address handlers - FIXED: Preserve address ID and update properly
  // const handleAddressSelect = (addressData) => {
  //   // Preserve the original address ID while updating other fields
  //   const updatedAddress = {
  //     ...editedAddress,
  //     ...addressData,
  //     _id: editedAddress._id // Keep the original ID
  //   };
    
  //   setEditedAddress(updatedAddress);
  //   setShowAddressMap(false);
    
  //   // Auto-save address when selected from map
  //   const updatedOrder = {
  //     ...order,
  //     restaurant: {
  //       ...order.restaurant,
  ...
  //   if (isEditing && editingSection === 'address') {
  //     updateOrderDetails(orderId, { restaurantAddress: updatedAddress })
  //       .then(() => toast.success('Address saved to database'))
  //       .catch(err => {
  ...
  const handleAddressSelect = (addressData) => {
  // Preserve all original address fields while updating with new data
  const updatedAddress = {
    ...editedAddress, // Keep all existing fields
    ...addressData,   // Override with new data
    _id: editedAddress._id, // Keep the original ID
    // Ensure all required fields are preserved
    type: addressData.type || editedAddress.type,
    street: addressData.street || editedAddress.street,
    area: addressData.area || editedAddress.area,
    landmark: addressData.landmark || editedAddress.landmark,
    city: addressData.city || editedAddress.city,
    state: addressData.state || editedAddress.state,
    zip: addressData.zip || editedAddress.zip,
    country: addressData.country || editedAddress.country,
    location: addressData.location || editedAddress.location
  };
  ...
  // Auto-save to backend if in edit mode
  if (isEditing && editingSection === 'address') {
    updateOrderDetails(orderId, { restaurantAddress: updatedAddress })
      .then(() => toast.success('Address saved to database'))
      .catch(err => {
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-white/50 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
                        ...
                        <>
                          ...
                          {order.restaurant.address.location?.coordinates && (
                            <div className="text-gray-500 text-xs mt-1">
                              Coordinates: {order.restaurant.address.location.coordinates[0].toFixed(6)}, {order.restaurant.address.location.coordinates[1].toFixed(6)}
                            </div>
                          )}
                        </>
                      ) : "N/A"}
                  ...
                  <div className="bg-gradient-to-br from-pink-50 to-rose-50 p-2 rounded-lg border border-pink-100">
...
// Your working MapLocationPicker component
const MapLocationPicker = ({ 
  ...
  if (!mapRef.current && mapContainer.current) {
    console.log('Initializing map with existing address:', existingAddress);
    ...
    // Add geolocate control (but don't auto-trigger if we have existing address)
    ...
    if (existingAddress?.location?.coordinates) {
      ...
      console.log('Adding marker at existing location:', lng, lat);
...
}, [existingAddress]); // Only depend on existingAddress
...
// Compact Reusable Components (keep the same as before)
