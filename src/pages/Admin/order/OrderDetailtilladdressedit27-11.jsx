...
const OrderDetails = () => {
  ...
  // Form states for editing - CHANGED: editedAddress now for deliveryAddress (customer)
  ...
  const fetchOrderDetails = useCallback(async () => {
    try {
      ...
      if (response) {
        ...
        // CHANGED: Initialize with customer delivery address
  ...
  const saveEditing = async () => {
    try {
      ...
      if (editingSection === 'additionalInfo') {
        ...
        // Also update deliveryLocation if changed
  ...
  // CHANGED: Handle customer delivery address select from map
  // const handleAddressSelect = (addressData) => {
  //   // Preserve all original address fields while updating with new data
  //   const updatedAddress = {
  //     ...editedDeliveryAddress, // Keep all existing fields
  //     ...addressData, // Override with new data
  //     _id: editedDeliveryAddress._id, // Keep the original ID
  //     // Ensure all required fields are preserved
  //     type: addressData.type || editedDeliveryAddress.type,
  ...
  // };
    ...
    try {
      ...
      const updatedAddress = {
        ...
        city: addressData.city || editedDeliveryAddress.city,
        state: addressData.state || editedDeliveryAddress.state,
        zip: addressData.zip || editedDeliveryAddress.zip, // Use zip instead of pincode
        pincode: addressData.zip || editedDeliveryAddress.pincode,
        country: addressData.country || editedDeliveryAddress.country
      };
      ...
      // NEW: Trigger the addressService update for the customer's standalone address
      ...
      if (userId && addressId) {
        console.log('🚀 Calling addressService.updateCustomerAddress...');
        ...
        const updatePayload = {
          ...
          zip: updatedAddress.zip || updatedAddress.pincode, // Use zip field
          country: updatedAddress.country,
          longitude: addressData.location?.coordinates[0] || order.deliveryLocation?.coordinates[0], // Extract longitude
          latitude: addressData.location?.coordinates[1] || order.deliveryLocation?.coordinates[1]  // Extract latitude
        };
        ...
        console.warn('Missing userId or addressId - skipping customer address update');
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-rose-50">
          ...
          <div className="xl:col-span-2 space-y-3">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-white/50 overflow-hidden">
              ...
              <div className="p-3">
                ...
                {/* CHANGED: Address & Instructions - Now shows Customer Delivery Address (editable) + Store Address (read-only) */}
      ...
      {/* Address Map Modal - CHANGED: Now for customer delivery address */}
      {showAddressMap && (
        <MapLocationPicker
          onSave={handleAddressSelect}
          onClose={() => setShowAddressMap(false)}
          selectedCustomer={order.customer}
          existingAddress={order.deliveryAddress}  // Customer delivery address
          existingLocation={order.deliveryLocation} // Customer coordinates
          editMode={true}
        />
      )}
    </div>
...
// Your working MapLocationPicker component - UNCHANGED, but now receives customer data
